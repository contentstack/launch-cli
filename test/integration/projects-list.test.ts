...
describe('integration: GET /projects', () => {
  ...
  it('sends the documented method, path, query string and headers', async () => {
    ...
    await buildApi(buildClient(), UNUSED_CMA, {} as never).projects.list({ org: ORG_UID, limit: 50, skip: 0 });
  ...
  it('omits limit and skip from the query string when they are not supplied', async () => {
    ...
    await buildApi(buildClient(), UNUSED_CMA, {} as never).projects.list({ org: ORG_UID });
  ...
  it('parses the documented bare envelope into a ProjectsPage', async () => {
    ...
    const page: ProjectsPage = await buildApi(buildClient(), UNUSED_CMA, {} as never).projects.list({ org: ORG_UID });
  ...
  it('renders the parsed page through the real table and pagination renderers', async () => {
    ...
    const page = await buildApi(buildClient(), UNUSED_CMA, {} as never).projects.list({ org: ORG_UID });
